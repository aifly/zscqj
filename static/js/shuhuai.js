...
/***/ function(module, exports, __webpack_require__) {
	...
	var data = {
	    ...
	    cityCanvas: (0, _utilMethodEs6.$)("#city-canvas"),
	    mask: (0, _utilMethodEs6.$)("#fly-mask"),
	    close: (0, _utilMethodEs6.$)("#fly-mask .fly-close"),
	    provinceData: [{
	        type: 'image',
	...
	var util = {
	    init: function init() {
	        ...
	        this.setSize();
	        var w = parseFloat(_utilMethodEs6.utilMethods.getStyle((0, _utilMethodEs6.$)('#fly-main .fly-panner-container')).width),
	        ...
	        this.bindEvent();
	        var d = this.renderLoading(data.canvas, "province");
	        ...
	        var BitMap = (function () {
	            ...
	            _createClass(BitMap, [{
	                ...
	                value: function render() {
	                    var s = this;
	                    var scale = (0, _utilMethodEs6.$)("#province-canvas").height / s.k;
	                    ...
	                    bit.on('mousedown', function (e) {
	                        ...
	                        var cityData = s.container.type === 'province' ? data.provinceData : data.cityData;

	                        if (bit.index > 0) {
	                            (function () {
	                                //bit.index = 0 的是线条和背景图片
	                                var dt = cityData[bit.index - 1];

	                                Array.from((0, _utilMethodEs6.$$)('.fly-container', data.mask)).forEach(function (item) {
	                                    item.style.display = 'none';
	                                });

	                                switch (dt.type) {
	                                    case "image":
	                                        self.initMaskContent(dt.type, dt.src, function () {
	                                            var img = (0, _utilMethodEs6.$)('.fly-image-container .fly-content img');
	                                            img.style.opacity = 1;
	                                            img.src = dt.src;
	                                    ...
	                                    case "text":
	                                        ...
	                                        (0, _utilMethodEs6.$)('#fly-text-container').innerHTML = dt.content;
	                                        break;
	                                    case "video":
	                                        self.initMaskContent(dt.type);
	                                        var index = s.container.type === "province" ? 0 : 1;
	                                        (0, _utilMethodEs6.$$)("video", data.mask)[index].style.display = 'block';
	                                        (0, _utilMethodEs6.$$)("video", data.mask)[index].play();
	                                        break;
	                                }

	                                (0, _utilMethodEs6.$)('.fly-' + dt.type + '-container .fly-city').innerHTML = dt.city;

	                                (0, _utilMethodEs6.$)('.fly-' + dt.type + '-container').style.display = 'block';
	                            })();
	                        }
	                    });
	                    s.bit = bit;
	        ...
	        this.proviceArr = [];
	        _utilMethodEs6.utilMethods.loading(imgArr, function (p, s) {
	            d.prec.text = Math.round(p * 100) + "%";
	        }, function (s) {
	            imgArr.forEach(function (img) {
	                _this.proviceArr.push(new BitMap({ src: img, container: d.container, stage: d.stage }));
	            });
	            d.stage.removeChild(d.prec, d.text);

	            createjs.Tween.get(d.container, { loop: false }).to({ alpha: 1 }, 1500, createjs.Ease.linear).call(function () {
	                setTimeout(function () {
	                    a = 1;
	                }, 500);
	            });
	    ...
	    loadingCity: function loadingCity() {
	        var _this2 = this;

	        if (!this.theCityIsLoaded) {
	            (function () {
	                _this2.theCityIsLoaded = true;
	                _this2.b = 1;
	                var cityImgArr = ['./static/images/city-bg.png'];
	                for (var i = 1; i < 45; i++) {
	                    cityImgArr.push('./static/images/c-' + i + '.png');
	                }
	                var d1 = _this2.renderLoading(data.cityCanvas, 'city', 1595, 670);
	                _this2.d1 = d1;
	                _this2.cityArr = [];
	                _utilMethodEs6.utilMethods.loading(cityImgArr, function (p, s) {
	                    d1.prec.text = Math.round(p * 100) + "%";
	                }, function () {

	                    cityImgArr.forEach(function (s) {
	                        _this2.cityArr.push(new _this2.BitMap({ src: s, k: 750, container: d1.container, stage: d1.stage }));
	                    });
	    ...
	    renderLoading: function renderLoading(canvas, type) {
	        var w = arguments.length <= 2 || arguments[2] === undefined ? 1700 : arguments[2];
	        var h = arguments.length <= 3 || arguments[3] === undefined ? 800 : arguments[3];
	        ...
	        //text.textBaseline = "top";
	        var cX = (canvas.width - canvas.height / h * w) / 2;
	    ...
	    setSize: function setSize() {
	        ...
	        var h = height - parseFloat(_utilMethodEs6.utilMethods.getStyle(data.logo).marginTop) - 30 - data.header.offsetHeight;
	        data.pannelC.style.height = h + 'px';
	        (0, _utilMethodEs6.$)('#fly-main .fly-logo').style.transform = 'scale(' + width / 1920 + ')';

	        Array.from((0, _utilMethodEs6.$$)("canvas")).forEach(function (item) {
	    ...
	    bindEvent: function bindEvent() {
	        ...
	        document.addEventListener("keydown", function (e) {
	            if (e.keyCode === 37 || e.keyCode === 39) {
	                cIndex = iNow % 2 === 0 ? 1 : 0;
	                (0, _utilMethodEs6.$$)(".nav")[cIndex].click();
	                iNow++;
	        ...
	        data.close.addEventListener('click', function () {
	            ...
	            var img = (0, _utilMethodEs6.$)('.fly-image-container .fly-content img', data.mask);
	            img.style.opacity = 0;
	            setTimeout(function () {
	                img.classList.add('loading-gif');
	                img.src = './static/images/loading1.gif';
	            }, 400);
	            Array.from((0, _utilMethodEs6.$$)('video', data.mask)).forEach(function (video) {
	                video.style.display = 'none';
	                video.pause();
	            });
	        });
	    },
	    initMaskContent: function initMaskContent(type, src, fn) {
	        var _this4 = this;

	        if (type === 'image') {
	            (function () {
	                var content = (0, _utilMethodEs6.$)('.fly-image-container .fly-content', data.mask);

	                _this4.imgLoad(src, function (w, h) {
	                    fn && fn();
	                    var cH = content.offsetHeight;
	                    var cW = content.offsetWidth;
	                    var img = (0, _utilMethodEs6.$)(".fly-content img", data.mask);
	                    if (w / h >= cW / cH) {
	                        img.style.width = "100%";
	    ...
	    imgLoad: function imgLoad(src, fn) {
	        var img = new Image();
	        img.onload = function () {
